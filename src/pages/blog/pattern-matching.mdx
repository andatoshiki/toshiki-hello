...
let greet name =
  match name with
  | "toshiki" -> print_string "Hi, toshiki!"
...
The first pattern is `"toshiki" -> print_string "Hi, toshiki!"`, which means that if `name` is equal to `"toshiki"`, it will execute the print statement. Similarly, if `name` is equal to `"Ludwig"` it will execute the respective print statement. The last pattern is `_ -> print_string "Who are you?"`, which means that if `name` is anything other than "toshiki" or "Ludwig", it will print "Who are you?".
